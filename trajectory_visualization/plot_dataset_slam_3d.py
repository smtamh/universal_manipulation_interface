...
def main(demo, output, include_lost, simultaneous, show):
    ...
    output_path = ensure_output_path(output, demo_root, "plot_dataset_tag_3d")
    ...
    for chunk_idx, chunk in enumerate(series_chunks):
        ...
        save_3d_plot(
            ...
            title=title_for_chunk(f"Tag-Frame 3D Trajectories ({demo_root})", chunk_idx, len(series_chunks)),
            show=show,
        )
