...
def main(demo, output, include_lost, simultaneous, show):
    demo_root = pathlib.Path(demo)
    series_list = [build_real_series(demo_dir) for demo_dir in find_demo_dirs(demo_root)]
    series_chunks = split_series_list(series_list, simultaneous)
    output_path = ensure_output_path(output, demo_root, "plot_dataset_real_3d")
    ...
    for chunk_idx, chunk in enumerate(series_chunks):
        ...
        save_3d_plot(
            ...
            title=title_for_chunk(f"Relative Real-Frame 3D Trajectories ({demo_root})", chunk_idx, len(series_chunks)),
            show=show,
        )
