...
def main(demo, output, include_lost, simultaneous):
    ...
    output_path = ensure_output_path(output, demo_root, "plot_dataset_real_2d")
    ...
    for chunk_idx, chunk in enumerate(series_chunks):
        ...
        save_2d_plot(
            ...
            title=title_for_chunk(f"Relative Real-Frame XYZ Trajectories ({demo_root})", chunk_idx, len(series_chunks)),
        )
